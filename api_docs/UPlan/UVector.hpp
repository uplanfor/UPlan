...
public:
    ...
    // to construct a UVector that will be filled with vals
    // @param size the number of the vals
    // @param val the elem to fill the vector
    UVector(const size_t &size, const T &val = T()) : UVector()
    {
        resize(size, val);
    }

    // copy constructor
    UVector(const UVector<T> &copy) : UVector()
    {
        reserve(copy.size);
    ...
    // to ger the reference of the first elem
    ...
    // to get the reference of the last elem
    // @return the reference of the last elem
    T& back() const
    {
        return m_last[-1];
    }
